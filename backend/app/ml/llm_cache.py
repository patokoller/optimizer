...
def prompt_fingerprint(prompt: str) -> str:
    """Stable sha256 of the prompt text plus the scoring-logic version, so a
    change in how the prompt is consumed (not just its text) invalidates the
    cache exactly once."""
    payload = f"{_SCORING_LOGIC_VERSION}\x00{prompt or ''}"
